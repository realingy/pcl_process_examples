...
int main(int argc, char** argv)
{
	...
	pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients()); //模型系数对象
	pcl::PointIndices::Ptr inliers(new pcl::PointIndices()); //点云索引对象
	int i = 0;
	while (cloud_filtered->points.size() > 0.3 * nr_points)
	{
		// 为了处理点云包含的多个模型，在一个循环中执行该过程并在每次模型被提取后，保存剩余的点
		...
		extract.setNegative(false); // false: 保留索引点，去除其他部分
		...
		extract.setNegative(true); // true: 去除索引点，保留其他部分
